...
//
	...
	if($result != ''){ $update_search = '<span class="update_search"><a id="'.$search_id.'" onclick="save_search(this.id)">Update saved search</a></span>'; }
	}
	...
	$rec_dropdown_broadcast .= "<option value=\"$obj->id\">$obj->e2location</option>"; 
	}
	}
	}
	...
	$result_list = $result_list."
		<div id=\"search_result_header\"><a href=\"search.php?searchterm=$raw_term&option=$option&search_channel=on&channel_id=$obj->e2eventservicereference&device=$device\" target=\"_blank\" class=\"links\" title=\"Search the term only on this channel\"><strong>$obj->e2eventservicename</strong></a>
		...
		<input id=\"searchlist_timer_btn_$obj->hash\" type=\"submit\" onClick=\"searchlist_timer(this.id,'record')\" value=\"SET TIMER\" class=\"btn btn-success btn-xs\" title=\"Send timer to Receiver\"/>
		<input id=\"searchlist_timer_btn_$obj->hash\" type=\"submit\" onClick=\"searchlist_timer(this.id,'zap')\" value=\"ZAP TIMER\" class=\"btn btn-warning btn-xs\" title=\"Send zap timer to Receiver\"/>
		<input id=\"searchlist_zap_btn_$obj->hash\" type=\"submit\" name=\"$obj->e2eventservicereference\" onClick=\"searchlist_zap(this.id,this.name)\" value=\"ZAP TO CHANNEL\" class=\"btn btn-default btn-xs\"/>
...
<a id="top"></a>
<div id="scroll_top"><a href="#" title="top">
...
<div id="remote_modal" class="modal_rc">
  <div class="modal-dialog animated">
    <div class="modal-content">
      <div class="modal-header">Remote Control <span id="rc_status"><i class="fa fa-wifi gray"></i></span> </div>
      <div class="modal-body">
        <div id="rc_frame"></div>
        <hr>
        <div align="right">
  ...
  <div class="navbar navbar-inverse navbar-fixed-top">
    <div class="adjust-nav">
      <div class="navbar-header">
        ...
        <span class="navbar-brand"><span style="font-size:1.1em; color:#000; cursor:pointer;" onclick="remote_modal.open();" title="Remote Control"><i class="fa fa-wifi fa-1x"></i></span> <a class="navbar-link" href="search.php">Search</a></span> </div>
    ...
    <div id="page-inner">
      <div class="row">
        <div id="cookie_js" class="col-md-12" style="color:#FF0000;">
          <noscript>
          To use all functions of the website, it's required to activate JavaScript.
          </noscript>
        </div>
      </div>
      <div class="row">
      ...
      <!--crawl channel id-->
      <div id="div_crawl_channel_id"> <span class="panel-close" onclick="animatedcollapse.hide('div_crawl_channel_id')"><span aria-hidden="true">x</span></span>
      ...
      <!--crawl complete-->
      <div id="div_crawl_complete"> <span class="panel-close" onclick="animatedcollapse.hide('div_crawl_complete')"><span aria-hidden="true">x</span></span>
      ...
      <!--crawl complete-->
      <!--crawl saved search-->
      <div id="div_crawl_search"> <span class="panel-close" onclick="animatedcollapse.hide('div_crawl_search')"><span aria-hidden="true">x</span></span>
      ...
      <!--crawl saved search-->
      <!--send timer-->
      <div id="div_send_timer"> <span class="panel-close" onclick="animatedcollapse.hide('div_send_timer')"><span aria-hidden="true">x</span></span>
      ...
      <!--send timer-->
      <!--channelzapper-->
      <div id="div_start_channelzapper"> <span class="panel-close" onclick="animatedcollapse.hide('div_start_channelzapper')"><span aria-hidden="true">x</span></span>
      ...
      <div class="row">
        <form method="#" action="">
          <div class="col-md-6">
            ...
            <div id="btn-group">
              ...
              <div id="btn2">Record location:
                <select class="rec_location_search_list" name="record_location" id="searchlist_record_location">
                  <?php 
				  	// record locations dropdown
					if($device == ''){ $device_number = '0'; } else { $device_number = $device; }
					$sql = "SELECT * FROM `record_locations` WHERE `device` = '".$device_number."' ORDER BY `id` ASC";
					if ($result = mysqli_query($dbmysqli,$sql))
					{
					while ($obj = mysqli_fetch_object($result)) {
					{
					// set selected
					if($obj->selected == '1'){ $select = 'selected="selected"'; } else { $select = ''; }
              ...
              <div id="btn-group-2">
                <div id="btn3">
                  <label>
                  <input type="checkbox" name="search_channel" id="search_channel" onclick="check_channel_search()" <?php if ($channel_id != ''){ echo 'checked'; } ?> />
                  Search only at this channel</label>
                </div>
                <div id="btn4"> Receiver:
                  <select name="device" id="search_list_device" class="search_list_device" onChange="search_list_change_device();">
					...
					if($device == $id){ $select = 'selected="selected"'; } else { $select = ''; }
					if(!isset($device_dropdown) or $device_dropdown == ''){ $device_dropdown = ''; }
					echo '<option value="'.$id.'" '.$select.'>'.$device_description.'</option>'; 
					}
					}
					}
					?>
          ...
          <div class="col-md-6"> Excluded term(s): <i class="glyphicon glyphicon-question-sign fa-1x" style="cursor:default;" title="Seperate terms with ; (semicolon)"></i> <span id="status_exclude_channel"></span> <span id="status_exclude_title"></span> <span id="status_exclude_description"></span> <span id="status_exclude_extdescription"></span>
            <div class="spacer_5"></div>
            <label>
            <input id="exclude_channel_checkbox" type="checkbox" onclick="check_exclude()" <?php if($exclude_channel !== ''){ echo 'checked'; } ?>>
            </label>
            <input name="exclude_channel" id="exclude_channel" type="text" style="width:80%;" value="<?php echo $exclude_channel; ?>" placeholder="Channel">
            <!---->
            <div class="spacer_5"></div>
            <label>
            <input id="exclude_title_checkbox" type="checkbox" onclick="check_exclude()" <?php if($exclude_title !== ''){ echo 'checked'; } ?>>
            </label>
            <input name="exclude_title" id="exclude_title" type="text" style="width:80%;" value="<?php echo $exclude_title; ?>" placeholder="Title">
            <!---->
            <div class="spacer_5"></div>
            <label>
            <input id="exclude_description_checkbox" type="checkbox" onclick="check_exclude()" <?php if($exclude_description !== ''){ echo 'checked'; } ?>>
            </label>
            <input name="exclude_description" id="exclude_description" type="text" style="width:80%;" value="<?php echo $exclude_description; ?>" placeholder="Description">
            <!---->
            <div class="spacer_5"></div>
            <label>
            <input id="exclude_extdescription_checkbox" type="checkbox" onclick="check_exclude()" <?php if($exclude_extdescription !== ''){ echo 'checked'; } ?>>
            </label>
            <input name="exclude_extdescription" id="exclude_extdescription" type="text" style="width:80%;" value="<?php echo $exclude_extdescription; ?>" placeholder="Extended description">
            <div class="spacer_10"></div>
            <label>
            <input name="rec_replay" id="rec_replay" type="checkbox" <?php if($rec_replay == 'on'){ echo 'checked'; } ?>>
            Set also timer for repeating broadcast's</label>
            <div class="spacer_10"></div>
          </div>
          <!--exclude-->
          <input name="search_id" id="search_id" type="text" class="hidden" value="<?php echo $search_id; ?>" placeholder="Extended description">
        </form>
      </div>
      <!-- /. ROW  -->
      <hr />
...
function save_search(id){
	...
	if(record_location == ''){ record_location = $("#searchlist_record_location option:selected").val(); }
	...
	device: '<?php echo $_REQUEST['device']; ?>', 
