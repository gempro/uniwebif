...
//
	...
	// sort saved search
	if(!isset($_REQUEST['sort_list']) or $_REQUEST['sort_list'] == ''){ $sortby = $search_list_sort; 
	...
	// record location
	$sql_1 = "SELECT * FROM `record_locations` WHERE `device` = '0' ORDER BY `id` ASC";
	...
	$record_dropdown .= '<option value="'.$obj->id.'">'.$obj->e2location.'</option>';
	}
	}
	}
	...
	$channel_dropdown .= '<option value="'.$obj->e2servicereference.'">'.$servicename_enc.'</option>';
	}
	}
	}
	
	// device dropdown, color, description
	$sql_3 = "SELECT * FROM `device_list` ORDER BY `id` ASC";
	if ($result_3 = mysqli_query($dbmysqli,$sql_3))
	{
	$device_desc[0] = 'default';
	$device_color[0] = '#ddd';
	$device_dropdown = '';
	...
	$device_desc[$obj->id] = $device_description;
	$device_color[$obj->id] = $obj->device_color;
	...
	// saved search list
	$sql_4 = "SELECT * FROM `saved_search` ORDER BY ".$sortby." ".$sortby1." ";
	if ($result_4 = mysqli_query($dbmysqli,$sql_4))
	{
	while ($obj = mysqli_fetch_object($result_4)) {
	...
	if($activ == 'yes'){ $active_checkbox = 'checked'; } else { $active_checkbox = ''; }
	...
	if($search_option == 'extdescription'){ $search_option_desc = 'Ext. description'; }
	...
	{
	if($group == strtoupper($searchterm[0][0])){ $group_title = ''; 
	} else { 
	$group_title = strtoupper($obj->searchterm[0][0]);
	...
	// record location id
	$sql_5 = mysqli_query($dbmysqli, "SELECT `id` FROM `record_locations` WHERE `device` = '".$obj->device."' AND `e2location` = '".$record_location."' LIMIT 0,1");
	$result_5 = mysqli_fetch_assoc($sql_5);
	$rec_location_id = $result_5['id'];
	
	// record location different device
	if($obj->device != 0)
	{
	$record_dropdown = '';
	$sql_6 = "SELECT * FROM `record_locations` WHERE `device` = '".$obj->device."' ORDER BY `id` ASC";
	if ($result_6 = mysqli_query($dbmysqli,$sql_6))
	{
	while ($obj_1 = mysqli_fetch_object($result_6)) {	
	{
	if(!isset($record_dropdown) or $record_dropdown == ''){ $record_dropdown = ''; }
	$record_dropdown .= '<option value="'.$obj_1->id.'">'.$obj_1->e2location.'</option>';
	}
	}
	}
	}
	...
	<div id=\"saved_search_list\" style=\"border: 1px solid $device_color[$device] !important;\">
	  <div class=\"row\">
	  ...
	  <div id=\"saved_search_list_div_$obj->id\" style=\"display:none;\">
		  ...
		  <div class=\"col-md-1\">
		  <p class=\"saved_search_chkbox\"><input id=\"active_$obj->id\" type=\"checkbox\" id=\"checkbox\" $active_checkbox>
		  <span class=\"chkbox_info\">Activate / Deactivate</span>
		  </p>
		  </div>
		  </div>
		  <div class=\"row\">
		  ...
		  <div class=\"col-md-2\">
			<p>Record device<br>
			<select id=\"rec_device_$obj->id\" class=\"device_list\" onChange=\"saved_search_change_device(this.id)\">
			<option selected disabled>$device_desc[$device]</option>
