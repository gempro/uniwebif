...
include("../inc/dashboard_config.php");
	...
	$searchterm_r = utf8_decode($_REQUEST['searchterm']);
...
//	if ($exclude_extdescription !== ''){ $exclude_extdescription = $exclude_extdescription; } else { $exclude_extdescription = ''; }
	...
	// get channel name
	$sql = mysqli_query($dbmysqli, "SELECT e2servicename, servicename_enc FROM `channel_list` WHERE `e2servicereference` = '".$e2servicereference."' LIMIT 0 , 1");
	...
	// get record path
	$sql = mysqli_query($dbmysqli, "SELECT `e2location` FROM `record_locations` WHERE `id` = '".$record_location."' LIMIT 0 , 1");
	$result = mysqli_fetch_assoc($sql);
	...
	crawled = '0',
	save_date = '".$save_date."', 
	e2eventservicereference = '".$e2servicereference."', 
	e2eventservicename = '".$e2servicename."', 
	...
	if($action == 'save')
	{
	mysqli_query($dbmysqli, "INSERT INTO `saved_search` 
   (
   searchterm, 
   searchterm_r, 
   search_option, 
